    ...
    def forward(self, init_features, edge_weight, noise_info, hx_init, cons, x_hat_MMSE, var_MMSE):
        ...
        for i in range(self.user_num):
            for j in range(self.user_num):
                if j!=i:
                    edge_embed[:,i,j,0] = edge_weight[:, counter]
                    edge_embed[:,i,j,1] = noise_info.view(batch_size)
                    counter += 1

        n_gnn_iters = self.num_iter
        ...
        for iter_l in range(n_gnn_iters):
            ...
            vn_val = self.aggregation(in_msgs=msg, agg_embed=t.zeros(size=(batch_size,self.user_num,0), device=self.device))
